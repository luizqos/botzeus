...
async function getLoginCookies() {
  const browser = await puppeteer.launch({ headless: Boolean(parseInt(process.env.HEADLESS)) });
  const page = await browser.newPage();
  const timeBrowser = process.env.TIME_BROWSER;

  try {
    // Acessa a página de login
    await page.goto(process.env.URL_ZEUS + '/login', { waitUntil: 'networkidle2' });
    ...
    console.log(`Navegador permanecerá aberto por ${parseInt(timeBrowser)/60000} minuto(s)...`);
    setTimeout(async () => {
